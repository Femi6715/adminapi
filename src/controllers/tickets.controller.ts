...
export class TicketsController {
  ...
  getTickets = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const tickets = await this.ticketsModel.getAllTickets(
        startDate as string,
        endDate as string
      );
  ...
  getTicketStats = async (req: Request, res: Response) => {
    try {
      const stats = await this.ticketsModel.getTicketStats();
      res.json(stats);
    } catch (error) {
  ...
  getTicketById = async (req: Request, res: Response) => {
    try {
      ...
      const ticket = await this.ticketsModel.getTicketById(ticketId);
  ...
  updateTicketStatus = async (req: Request, res: Response) => {
    try {
      ...
      const success = await this.ticketsModel.updateTicketStatus(ticketId, status);
      ...
      const updatedTicket = await this.ticketsModel.getTicketById(ticketId);
      if (!updatedTicket) {
  ...
  deleteTicket = async (req: Request, res: Response) => {
    try {
      ...
      const deletedTicket = await this.ticketsModel.deleteTicket(ticketId);
      if (!deletedTicket) {
        return res.status(404).json({ error: 'Ticket not found' });
      }
