...
export class TicketsController {
  ...
  async updateTicketStatus(req: Request, res: Response) {
    try {
      ...
      const [result] = await pool.query(
        'UPDATE tickets SET ticket_status = ?, updated_at = CURRENT_TIMESTAMP WHERE ticket_id = ?',
        [status, id]
      );
      ...
      const [updatedTickets] = await pool.query<Ticket[]>(
        'SELECT * FROM tickets WHERE ticket_id = ?',
        [id]
      );
  ...
  async getWeeklyTickets(req: Request, res: Response) {
    try {
      ...
      const [allTickets] = await pool.query<CountResult[]>(
      ...
      const [dates] = await pool.query<DateResult[]>(
        'SELECT CURDATE() as current_date, DATE_SUB(CURDATE(), INTERVAL 7 DAY) as seven_days_ago'
      );
      ...
      if (rows.length > 0) {
        ...
        const [recentTickets] = await pool.query<Ticket[]>(
          'SELECT * FROM tickets ORDER BY created_at DESC LIMIT 5'
        );
        if (recentTickets.length > 0) {
          console.log('Most recent tickets (not in weekly range):', 
            recentTickets.map(t => ({
              id: t.id,
  ...
  async getMonthlyTickets(req: Request, res: Response) {
    try {
      ...
      const [rows] = await pool.query<Ticket[]>(
        'SELECT * FROM tickets WHERE created_at >= CURDATE() - INTERVAL 30 DAY ORDER BY created_at DESC'
      );
